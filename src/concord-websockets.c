...
static jscon_item_t*
_concord_identify_init(char token[])
{
    uv_utsname_t buffer;
    int uvcode = uv_os_uname(&buffer);
    ASSERT_S(!uvcode, "Couldn't fetch system information");
    ...
    jscon_append(event_data, jscon_string("token", token));
    ...
    jscon_item_t *identify = jscon_object(NULL);
    jscon_append(identify, jscon_integer("op", GATEWAY_IDENTIFY));
    jscon_append(identify, event_data);
...
Concord_ws_init(char token[])
{
    ...
    new_ws->identify = _concord_identify_init(token);
...
{
    ASSERT_S(!uvstatus, uv_strerror(uvstatus));
...
{
    D_PRINT("TIMEOUT_MS: %ld", timeout_ms);
...
Concord_ws_socket_cb(CURL *easy_handle, curl_socket_t sockfd, int action, void *p_userdata, void *p_socket)
{
    ...
    case CURL_POLL_REMOVE:
        ...
        break;
    default:
        ERROR("Unknown CURL_POLL_XXXX option encountered\n\tCode: %d", action);
    }
...
_uv_on_heartbeat_cb(uv_timer_t *req)
{
    ...
    D_PRINT("REPEAT_MS: %ld", uv_timer_get_repeat(&ws->heartbeat_timer));
...
_concord_on_ws_hello(concord_ws_t *ws)
{
    ...
    ASSERT_S(heartbeat_ms > 0, "Invalid heartbeat_ms");
...
Concord_on_text_cb(void *data, CURL *easy_handle, const char *text, size_t len)
{
    ...
    case GATEWAY_HEARTBEAT_ACK:
          break; 
    default:
          ERROR("Found not yet implemented Gateway Opcode: %d", ws->payload.opcode);
    }
...
_uv_disconnect_cb(uv_timer_t *req)
{
    ...
    ASSERT_S(true == ret, "Couldn't disconnect gracefully from WebSockets");
...
_uv_on_force_close_cb(uv_async_t *req)
{
    ...
    D_PUTS("Attempting to disconnect from gateway ...");
